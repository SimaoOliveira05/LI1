...
teste01 :: Test
teste01 = "T01: Quando não há nenhuma acção, o jogo permanece inalterado" ~: jogo01 ~=? atualiza [] Nothing jogo01

andarDireita01 :: Jogo
andarDireita01 = atualiza [] (Just AndarDireita) jogo01
...
teste02 = TestLabel "T02" $ test [testeA, testeB]
  where
    ...
    resultadoAndarDireita = atualiza [] (Just AndarDireita) jogo01
...
teste03 = TestLabel "T03" $ test [testeA, testeB]
  where
    ...
    resultadoAndarDireita = atualiza [] (Just AndarEsquerda) jogo01

teste04 :: Test
teste04 = TestLabel "T04" $ test [testeA, testeB]
  where
    testeA = "A: Quando a acção é Saltar, o vetor velocidade do jogador é positivo na componente do Y" ~: True ~=? (snd . velocidade . jogador $ resultadoSaltar) > 0
    testeB = "B: Quando a acção é Saltar, a orientação do jogador não muda" ~: (direcao . jogador $ jogo01) ~=? (direcao . jogador $ resultadoSaltar)
    resultadoSaltar = atualiza [] (Just Saltar) jogo01
...
teste05 = TestLabel "T05" $ test [testeA]
  where
    testeA = "A: Quando a acção é Subir, o vetor velocidade do jogador é positivo na componente do Y" ~: True ~=? (snd . velocidade . jogador $ resultadoSubir) > 0
    resultadoSubir = atualiza [] (Just Subir) jogo02
...
teste06 :: Test
teste06 = TestLabel "T06" $ test [testeA]
  where
